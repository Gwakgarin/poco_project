...
public class LinkController {
    ...
    // 보호자가 코드 사용: body 예시 {"code":"ABCD1234","guardianId":2,"relationLabel":"FAMILY"}
    // relationLabel 값은 FAMILY | CAREGIVER | FRIEND | OTHER 중 하나여야 함 (대소문자 무관)
    @PostMapping("/redeem")
    public Map<String, Object> redeem(@RequestBody Map<String, Object> body) {
        ...
        String relationLabelRaw = (String) body.get("relationLabel");

        RelationLabel relationLabel = null;
        if (relationLabelRaw != null) {
            try {
                relationLabel = RelationLabel.valueOf(relationLabelRaw.trim().toUpperCase());
            } catch (IllegalArgumentException e) {
                return Map.of(
                        "success", false,
                        "message", "relationLabel 값이 올바르지 않습니다. FAMILY, CAREGIVER, FRIEND, OTHER 중 하나여야 합니다."
                );
            }
        }
